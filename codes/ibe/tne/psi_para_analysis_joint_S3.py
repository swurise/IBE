...
if __name__ == "__main__":
    for mydata in datasets:  # 遍历数据集 modify by huganglin 20200131
        for mymethod in methods:
            ...
            for psi_list in psi_coefficient_list:
                methodvector_path = basepath + '/' + mydata + '/method_vector/' + mymethod + '_train_sort_noid.txt'
                topicmatrix_path = basepath + '/' + mydata + '/topic_matrix/' + str(t) + '_topicMatrix.txt'
                jointvector_path = basepath + '/' + mydata + '/' + 'psi_parameter_analysis/' + mymethod + '/psi_' + str(psi_list) + '_joint.txt'
                ...
                alpha_coefficient = pow(auc_topic/auc_method,psi_list)
                abs_alpha_coefficient = pow(auc_topic / auc_method, abs(psi_list))
