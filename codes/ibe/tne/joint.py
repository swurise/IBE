...
#-------------AUC-----------------
import random
def auc(list_source, dataset):
    node2vec = {}
    for i in range(len(list_source)):
        node2vec[i] = list(map(float, list_source[i]))

    # huganglin
    # graphtestfile=basepath + '/' + dataset + '/graph_test.txt'
    graphtestfile=basepath + '/' + dataset + '/edgelistMergeFile/graph_train.txt'
...
def list_enlargement(list_source, large_coefficient):
    for i in range(len(list_source)):  # 行数
        for j in range(len(list_source[i])):  # 列数
            list_source[i][j] = float(list_source[i][j]) * int(large_coefficient)  # 遍历二维列表做数据转换之用
    return(list_source)

def list_max_str(mydata):
...
def read_Mean_List(filename):
    ...
    for i in range(len(list_source)):  # 行数
        for j in range(len(list_source[i])):  # 列数
            list_source[i][j]=abs(float(list_source[i][j]))
    file1.close()
...
if __name__ == "__main__":
    ...
    #     for mymethod in methods:  # 遍历数据集 modify by huganglin 20200131
            mydata='wiki'
            mymethod='LINE'
            t = list_max_str(mydata)
            methodvector_path = basepath + '/' + mydata + '/method_vector/' + mymethod + '_train_sort_noid.txt'
            topicmatrix_path = basepath + '/' + mydata + '/topic_matrix/' + t + '_topicMatrix.txt'
            jointvector_path = basepath + '/' + mydata + '/joint_vector/' + mymethod + '_' + t + '_joint.txt'
            method_list_source = read_List(methodvector_path)
            topic_list_source = read_List(topicmatrix_path)
            # a = read_Mean_List(methodvector_path)
            # b = read_Mean_List(topicmatrix_path)
            joint_coefficient = 0.7 #round(a / b)
            print('joint_coefficient:',joint_coefficient)

            topic_list_large = list_enlargement(topic_list_source, joint_coefficient)
            jointlist = mergeVec_ReturnList(topic_list_large,method_list_source)
            # print('method_list_source',auc(method_list_source, mydata))
            # print('topic_list_source',auc(topic_list_source, mydata))
            # print('jointlist',auc(jointlist, mydata))
