...
if __name__ == "__main__":
    for mydata in datasets:  # 遍历数据集 modify by huganglin 20200131
        for mymethod in methods:  # 遍历数据集 modify by huganglin 20200131
            train_all_vector = basepath + '/' + mydata + '/method_vector/' + mymethod + '_train_all.txt'
            train_all_sort = basepath + '/' + mydata + '/method_vector/' + mymethod + '_train_sort.txt'
            print(train_all_vector, ', ', train_all_sort)
            SortedPR,emb_dimen1 = Read_list_sort(train_all_vector)
            Save_list(SortedPR, emb_dimen1, train_all_sort)
